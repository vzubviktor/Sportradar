...
import {fetchTournaments, fetchMatches, dateConstructor, sortMatches} from "../components/Api";
...
// testing if our dateConstructor working properly
...
it( ' testing if sorting algorithm works,', async () =>{
const result = await fetchTournaments()// fetching matches from tournaments
.then((res) => res.map((tournament) =>{
  return tournament._tid
}))
.then((res) => res.map((tournamentID) =>{
    fetchMatches(tournamentID)
    .then((res) =>{
    const test = sortMatches(res); // start testing sorting algorithm
    expect(test.length <= 5).toBe(true);// check if we returned last 5 matches
    })
  }))
})
